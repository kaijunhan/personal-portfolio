import AnimatedText from "../components/AnimatedText";
import { motion, useMotionValue, useReducedMotion } from "framer-motion";
import { Helmet } from 'react-helmet';
import blog1 from "../images/articles/squirrel.png";
import blog2 from "../images/articles/otter.png";
import blog3 from "../images/articles/cat.png";
import blog4 from "../images/articles/deer.png";
import Navbar from "../components/Navbar";
import Layout from "../components/Layout";
import Footer from '../components/Footer';
import AccessibilityMotion from "../components/AccessibilityMotion";
import { Link } from "react-router-dom";
import { useRef } from "react";
import TransitionEffect from "../components/TransitionEffect";

const FramerImage = motion.img;

const MovingImg = ({ title, img, link }) => {
  const shouldReduceMotion = useReducedMotion();
  const x = useMotionValue(0);
  const y = useMotionValue(0);
  const imgRef = useRef(null);

  function handleMouse(event) {
    if (!shouldReduceMotion) {
      imgRef.current.style.display = "inline-block";
      x.set(event.pageX);
      y.set(event.pageY);
    }
  }

  function handleMouseLeave(event) {
    if (!shouldReduceMotion) {
      imgRef.current.style.display = "none";
      x.set(0);
      y.set(0);
    }
  }

  return (
    <>
      <Link
        href={link}
        target={"_blank"}
        className="relative"
        onMouseMove={handleMouse}
        onMouseLeave={handleMouseLeave}
      >
        <h2 className="capitalize text-xl font-semibold hover:underline dark:text-light md:text-lg xs:text-base">
          {title}
        </h2>
        <FramerImage
          src={img}
          ref={imgRef}
          alt={title}
          className="w-96 h-auto z-10 hidden absolute rounded-lg md:!hidden"
          initial={{ opacity: 0 }}
          animate={{ opacity: shouldReduceMotion ? 1 : 0 }}
          whileInView={{ opacity: shouldReduceMotion ? 1 : 0 }}
          transition={{ duration: 0.2 }}
          style={{
            x: x,
            y: y,
          }}
          sizes="(max-width: 768px) 60vw,
              (max-width: 1200px) 40vw,
              33vw"
        />
      </Link>
    </>
  );
};

const Article = ({ img, title, date, link }) => {
  const shouldReduceMotion = useReducedMotion();

  return (
    <motion.li
      initial={shouldReduceMotion ? {} : { y: 200 }}
      whileInView={shouldReduceMotion ? {} : { y: 0, transition: { duration: 0.5, ease: "easeInOut" } }}
      viewport={{ once: true }}
      className="relative w-full p-4 py-6 my-2 rounded-xl flex sm:flex-col items-center justify-between 
      bg-light text-dark first:mt-0 border border-solid border-dark
      border-r-4 border-b-4 dark:bg-dark dark:border-light
      "
    >
      <MovingImg img={img} title={title} link={link} />
      <span className="text-primary font-semibold dark:text-primaryDark min-w-max pl-4 sm:self-start 
      sm:pl-0 xs:text-sm">
        {date}
      </span>
    </motion.li>
  );
};

const FeaturedArticle = ({ img, title, time, summary, link }) => {
  const shouldReduceMotion = useReducedMotion();

  return (
    <li className="relative w-full p-4 col-span-1 bg-light border border-dark border-solid rounded-2xl 
    dark:bg-dark dark:border-light">
      <div
        className="absolute  top-0 -right-3 w-[102%] h-[103%] rounded-[2rem]  rounded-br-3xl bg-dark 
        -z-10  "
      />
      <Link
        href={link}
        target={"_blank"}
        className="inline-block w-full"
      >
        <FramerImage
          src={img}
          alt={title}
          className="w-full h-56 object-cover"
          whileHover={shouldReduceMotion ? {} : { scale: 1.05 }}
          transition={{ duration: 0.2 }}
          sizes="100vw"
          priority="true" 
        />
      </Link>

      <Link href={link} target={"_blank"}>
        <h2 className="capitalize text-2xl font-bold my-2 mt-4 hover:underline xs:text-lg">
          {title}
        </h2>
      </Link>
      <p className="text-sm  mb-2">{summary}</p>
      <span className="text-primary font-semibold dark:text-primaryDark">
        {time}
      </span>
    </li>
  );
};

export default function Articles() {
  const shouldReduceMotion = useReducedMotion();

  return (
    <main>
      <AccessibilityMotion />
      {shouldReduceMotion ? null : <TransitionEffect />}
      <div className="bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-300 min-h-screen font-inter">
        <div className="max-w-5xl w-11/12 mx-auto">
          <Helmet>
            <title>Kai's Portfolio Built with Nextjs | Blog Page</title>
            <meta name="description" content="Browse through CodeBucks's collection of software engineering articles and 
            tutorials on Next.js, React.js, web development, and more. 
            Gain valuable insights and stay up-to-date with SEO tips for building a developer portfolio." />
          </Helmet>
          <Navbar />
          <div
            className={`w-full mb-16 flex flex-col items-center justify-center dark:text-light overflow-hidden`}
          >
            <Layout className="pt-16 max-w-5xl mx-auto">
              <AnimatedText
                text="Welcome to my blog!"
                className="!text-8xl !leading-tight mb-16 lg:!text-7xl sm:!text-6xl xs:!text-4xl sm:mb-8"
              />

              <ul className="grid lg:grid-cols-2 gap-16 lg:gap-8 md:grid-cols-1 md:gap-y-16">
                <FeaturedArticle
                  img={blog1}
                  title="build a custom pagination component in reactjs from scratch"
                  time="9 min read"
                  summary="Learn how to build a custom pagination component in ReactJS from scratch. Follow this 
                  step-by-step guide to integrate Pagination component in your ReactJS project."
                  link="https://umich.edu"
                />

                <FeaturedArticle
                  img={blog2}
                  title="creating stunning loading screens in react: Build 3 types of loading screens"
                  time="10 min read"
                  summary="Learn how to create stunning loading screens in React with 3 different methods. 
                  Discover how to use React-Loading, React-Lottie & build a custom loading screen. Improve the 
                  user experience."
                  link="https://umich.edu"
                />
              </ul>

              <h2 className="font-bold text-4xl w-full text-center mt-32 mb-16 text-light">
                All Articles
              </h2>

              <ul className="flex flex-col items-center relative">
                <Article
                  title="form validation in reactjs: build a reusable custom hook for inputs and error handling"
                  img={blog3}
                  date="April 22, 2024"
                  link="https://umich.edu"
                />
                <Article
                  title="silky smooth scrolling in reactjs: a step-by-step guide for react developers"
                  img={blog4}
                  date="April 22, 2024"
                  link="https://umich.edu"
                />
              </ul>
            </Layout>
            </div>
          <Footer />
        </div>
      </div>
    </main>

  );
}